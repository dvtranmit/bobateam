...
[picture] = imread('normalmole4bit.bmp');
...
[picture color_table] = imread('normalmole4bit.bmp');
...
%scaled_data = red*255;              %scales the floats back to 0-255
%rounded_data = round(scaled_data);  %rounds them down
data = dec2bin(red,8);     %convert the binary data to 8 bit binary #s

%open a file
output_name = 'normal.coe';
file = fopen(output_name,'w');
