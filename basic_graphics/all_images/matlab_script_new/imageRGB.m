...
[picture color_table] = imread('happy.bmp');
...
data = dec2bin(rounded_data,4);

%open a file
output_name = 'happy_blue.coe';
...
red = pixel_columns;             %grabs the red part of the colortable
%scaled_data = red*255;              %scales the floats back to 0-255
%rounded_data = round(scaled_data);  %rounds them down
data = dec2bin(red,4);     %convert the binary data to 8 bit binary #s

%open a file
output_name = 'happy.coe';
file = fopen(output_name,'w');
